...
function buildPlainFeedback(name, age, grades) {
  // Эмодзи те же, что у оценок в виджете и в заголовках разделов у ChatGPT —
  // чтобы примечание выглядело одинаково независимо от того, сработал ли ИИ.
  const ICONS = {
    'Коммуникация': '🗣',
    'Работа в команде': '🤝',
    'Компьютерные навыки': '💻',
  ...
  const lines = ['📋 Обратная связь после открытого урока — ' + (name || '—') +
                 (age ? ' (' + age + ' лет)' : ''), ''];
  Object.keys(grades).forEach(k => {
    if (grades[k]) lines.push((ICONS[k] || '•') + ' ' + k + ': ' + grades[k]);
  });
  return lines.join('\n');
}
...
function generateFeedbackText(name, age, grades) {
  ...
  const prompt =
    'Ты методист IT академии Superkid. Напиши родителям развёрнутую обратную ' +
    'связь по итогам открытого урока, на русском языке.\n\n' +
    ...
    'Вторая строка — «Обратная связь после открытого урока ученика ' + childName + '. » ' +
    'и одно предложение о том, чем преподаватель остался доволен.\n' +
    'Далее разделы. Заголовок каждого — на отдельной строке, строго с этим ' +
    'эмодзи в начале:\n' +
    '  ✨ Общее впечатление — 3-4 предложения: активность и заинтересованность, ' +
    'как характер помогает в общении, сильные стороны, что полезно развивать.\n' +
    ...
    '  🚀 Рекомендация — 2-3 предложения: пригласи продолжить обучение в IT ' +
    'академии Superkid, назови сильные стороны ученика и что академия поможет ' +
    'усилить, упомяни программирование, критическое мышление и креативность.\n\n' +
    'ТОН И ПРАВИЛА:\n' +
    '- Пиши о преподавателе в третьем лице: «Преподаватель отметил…», ' +
    '«Преподаватель подчеркнул…».\n' +
    '- Тон доброжелательный и поддерживающий. Слабые стороны подавай как зону ' +
    'роста, а не как недостаток: не «плохо работает в команде», а «командные ' +
    'проекты помогут раскрыть потенциал».\n' +
    '- В каждом разделе упоминай, как академия будет развивать этот навык: ' +
    'мини-презентации, командные проекты, групповые обсуждения, мини-проекты, ' +
  ...
  const res = UrlFetchApp.fetch('https://api.openai.com/v1/chat/completions', {
    ...
    payload: JSON.stringify({
      ...
      temperature: 0.7,
      // Полный текст с шестью разделами — около 700-900 токенов;
      // при прежнем лимите 400 он обрывался на середине.
      max_tokens: 1600,
    }),
  });
  ...
  // Заголовок и разделы модель пишет сама — свой заголовок и список оценок
  // сверху не добавляем, иначе он дублировал бы её первые строки.
  // Звёздочки markdown убираем: в примечании AmoCRM разметка не отображается.
...
/**
 * Проверка текста обратной связи — ничего не пишет в AmoCRM.
 * Набор оценок повторяет эталонный пример по Эмилю, чтобы можно было
 * сравнить формат с тем, что выдавал прежний прокси.
 */
function debugFeedbackText() {
  const grades = {
    'Коммуникация': 'Активно общается, отвечает на вопросы',
    'Работа в команде': 'Хорошо работает в группе',
    'Компьютерные навыки': 'Уверенные',
    'Самостоятельность': 'Высокая',
    'Характер': 'Активный, эмоциональный',
  };
  Logger.log('--- Через ChatGPT ---');
  try {
    const t = generateFeedbackText('Эмиль', 10, grades);
    Logger.log(t || '(ключ OPENAI_API_KEY не задан — пойдёт текст без ИИ)');
  } catch (err) {
    Logger.log('Ошибка: ' + err);
  }
  Logger.log('');
  Logger.log('--- Запасной вариант, если ChatGPT недоступен ---');
  Logger.log(buildPlainFeedback('Эмиль', 10, grades));
}
