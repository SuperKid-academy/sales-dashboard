...
// ============================================================
// ОТМЕТКА ПОСЕЩЕНИЯ ОУ — ЗДЕСЬ БОЛЬШЕ НЕТ
// ============================================================
// Перевод сделки и обратная связь вынесены в отдельный сервис
// (папка amo-proxy в репозитории, развёрнут на Railway).
//
// Причина: дневная квота Google на исходящие запросы общая для этого
// скрипта. Синхронизация её выедала, и преподаватель во время урока
// получал «Служба была вызвана слишком много раз за день».
//
// Держать здесь второй обработчик нельзя ещё и потому, что вкладка со
// старым адресом двинула бы сделку повторно.
//
// Если веб-приложение было развёрнуто: Развернуть → Управление
// развёртываниями → архивировать, иначе старый адрес продолжит отвечать.
